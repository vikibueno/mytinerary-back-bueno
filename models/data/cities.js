...
async function createCities(arrayCities) {
    try {
        ...
        for (let city of arrayCities) {
            ...
            let admin_id = await user.id                         //solo necesito el id del usuario
            city.admin_id = admin_id 
            await City.create(city)                              //reasingno el valor del admin_id de cada ciudad para cambiar el mail que viene por default por LO QUE CORRESPONDE QUE SERIA EL ID
        }
        console.log('!done');
    } catch (error) {
        console.log(error);
